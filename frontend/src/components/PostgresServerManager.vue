<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h3 class="text-lg font-medium">Serveurs PostgreSQL</h3>
    ...
    <div v-if="servers.length === 0" class="text-center py-12 bg-muted/50 rounded-lg">
      ...
      <p class="text-muted-foreground mb-4">Configurez votre premier serveur PostgreSQL pour commencer</p>
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
              <div>
                ...
                <p v-if="server.database" class="text-xs text-muted-foreground">Base: {{ server.database }}</p>
              </div>
            </div>
            <div class="flex items-center gap-2">
              <Button
    ...
    <Dialog :open="showAddModal || !!editingServer" @update:open="(open: boolean) => !open && closeModal()">
      <DialogContent class="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{{ editingServer ? 'Modifier le serveur' : 'Ajouter un serveur PostgreSQL' }}</DialogTitle>
        </DialogHeader>
        <form @submit.prevent="saveServer" class="space-y-6">
          ...
          <div>
            <Label for="database">Base de données par défaut</Label>
            <Input
              v-model="formData.database"
              id="database"
              placeholder="postgres (par défaut)"
            />
            <p class="text-xs text-muted-foreground mt-1">
              Base de données utilisée par défaut pour la connexion
            </p>
          </div>
        ...
        <div class="space-y-6">
          <div>
            ...
            <textarea
              ...
              id="importData"
              rows="10"
              placeholder="Collez votre configuration JSON des serveurs PostgreSQL ici..."
            ></textarea>
          </div>
...
import PostgresDatabaseTransfer from './PostgresDatabaseTransfer.vue';

const servers = ref<PostgresServer[]>([]);
const showAddModal = ref(false);
const showImportModal = ref(false);
const showTransferModal = ref(false);
const sourceServerForTransfer = ref<PostgresServer | null>(null);
const editingServer = ref<PostgresServer | null>(null);
const serverToDelete = ref<PostgresServer | null>(null);
...
const editServer = (server: PostgresServer) => {
...
const saveServer = () => {
  if (editingServer.value) {
    ...
    PostgresServersManager.updateServer(editingServer.value.id, formData.value);
    toast.success('Serveur mis à jour avec succès');
  } else {
    // Add new server
    PostgresServersManager.addServer(formData.value);
...
const setAsDefault = (id: string) => {
  PostgresServersManager.setDefaultServer(id);
  loadServers();
  toast.success('Serveur par défaut mis à jour');
};

const confirmDelete = (server: PostgresServer) => {
  serverToDelete.value = server;
};

const deleteServer = () => {
  if (serverToDelete.value) {
    PostgresServersManager.deleteServer(serverToDelete.value.id);
    loadServers();
...
const testConnection = async (server: PostgresServer) => {
  const result = await PostgresServersManager.testConnection(server);
  if (result.success) {
...
const cloneServer = (server: PostgresServer) => {
  sourceServerForTransfer.value = server;
  showTransferModal.value = true;
};

const exportServers = () => {
  const json = PostgresServersManager.exportServers();
  const blob = new Blob([json], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `postgresql-servers-${new Date().toISOString().split('T')[0]}.json`;
  a.click();
...
const importServers = () => {
  const result = PostgresServersManager.importServers(importData.value, replaceOnImport.value);
  if (result.success) {
