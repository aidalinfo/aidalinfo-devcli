<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h3 class="text-lg font-medium">Serveurs MySQL</h3>
    ...
    <div v-if="servers.length === 0" class="text-center py-12 bg-muted/50 rounded-lg">
      ...
      <p class="text-muted-foreground mb-4">Configurez votre premier serveur MySQL pour commencer</p>
    ...
    <Dialog :open="showAddModal || !!editingServer" @update:open="(open: boolean) => !open && closeModal()">
      <DialogContent class="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{{ editingServer ? 'Modifier le serveur' : 'Ajouter un serveur MySQL' }}</DialogTitle>
        </DialogHeader>
        <form @submit.prevent="saveServer" class="space-y-6">
          <div>
            ...
            <Input
              ...
              id="name"
              required
              placeholder="ex: MySQL Production"
            />
          </div>
          ...
          <div>
            ...
            <Input
              ...
              id="auth-database"
              placeholder="mysql (par défaut)"
            />
            <p class="text-xs text-muted-foreground mt-1">
              Base de données utilisée pour l'authentification (généralement 'mysql')
            </p>
          </div>
        ...
        <div class="space-y-6">
          <div>
            ...
            <textarea
              ...
              id="importData"
              rows="10"
              placeholder="Collez votre configuration JSON des serveurs MySQL ici..."
...
import MySQLDatabaseTransfer from './MySQLDatabaseTransfer.vue';

const servers = ref<MySQLServer[]>([]);
const showAddModal = ref(false);
const showImportModal = ref(false);
const showTransferModal = ref(false);
const sourceServerForTransfer = ref<MySQLServer | null>(null);
const editingServer = ref<MySQLServer | null>(null);
const serverToDelete = ref<MySQLServer | null>(null);
...
const loadServers = () => {
  servers.value = MySQLServersManager.getServers();
};
...
const editServer = (server: MySQLServer) => {
...
const saveServer = () => {
  if (editingServer.value) {
    ...
    MySQLServersManager.updateServer(editingServer.value.id, formData.value);
    toast.success('Serveur mis à jour avec succès');
  } else {
    // Add new server
    MySQLServersManager.addServer(formData.value);
...
const setAsDefault = (id: string) => {
  MySQLServersManager.setDefaultServer(id);
  loadServers();
  toast.success('Serveur par défaut mis à jour');
};

const confirmDelete = (server: MySQLServer) => {
  serverToDelete.value = server;
};

const deleteServer = () => {
  if (serverToDelete.value) {
    MySQLServersManager.deleteServer(serverToDelete.value.id);
    loadServers();
...
const testConnection = async (server: MySQLServer) => {
  const result = await MySQLServersManager.testConnection(server);
  if (result.success) {
...
const cloneServer = (server: MySQLServer) => {
  sourceServerForTransfer.value = server;
  showTransferModal.value = true;
};

const exportServers = () => {
  const json = MySQLServersManager.exportServers();
  const blob = new Blob([json], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `mysql-servers-${new Date().toISOString().split('T')[0]}.json`;
  a.click();
...
const importServers = () => {
  const result = MySQLServersManager.importServers(importData.value, replaceOnImport.value);
  if (result.success) {
