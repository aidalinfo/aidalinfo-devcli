<template>
  <Dialog :open="open" @update:open="handleClose">
    <DialogContent class="sm:max-w-[700px] max-h-[90vh] flex flex-col">
      <DialogHeader>
        <DialogTitle>Transfert de base de données PostgreSQL</DialogTitle>
        <DialogDescription>
      ...
      <div class="space-y-6 overflow-y-auto flex-1 pr-2">
        ...
        <div v-if="destinationType === 'clone'" class="space-y-4 p-4 bg-muted/30 rounded-lg">
          ...
          <div>
            <Label for="clone-database">Base de données par défaut</Label>
            <Input
              v-model="clonedServer.database"
              id="clone-database"
              placeholder="postgres"
            />
          </div>
...
import PostgresServerSelector from './PostgresServerSelector.vue';
import { ListPostgresDatabases, TransferPostgresDatabase } from '../../wailsjs/go/main/App';

interface Props {
  open: boolean;
  sourceServer: PostgresServer;
}
...
const clonedServer = ref({
  ...
  database: props.sourceServer.database || 'postgres'
});
...
const canStartTransfer = computed(() => {
  ...
  if (destinationType.value === 'clone') {
    return clonedServer.value.name && clonedServer.value.host && clonedServer.value.port && clonedServer.value.user;
  } else {
    return selectedDestinationId.value !== '';
  }
});

const getDestinationServer = (): PostgresServer | null => {
  if (destinationType.value === 'clone') {
    return {
      id: 'temp-clone',
      ...
      database: clonedServer.value.database,
      isDefault: false
    };
  } else {
...
const loadDatabases = async () => {
  ...
  try {
    const result = await ListPostgresDatabases(
    ...
    databases.value = result.filter(db => !['postgres', 'template0', 'template1'].includes(db));
...
const startTransfer = async () => {
  ...
  if (destinationType.value === 'clone') {
    const newServer = PostgresServersManager.addServer({
      name: clonedServer.value.name,
      host: clonedServer.value.host,
      port: clonedServer.value.port,
      user: clonedServer.value.user,
  ...
  for (const database of selectedDatabases.value) {
    ...
    try {
      await TransferPostgresDatabase(
        props.sourceServer.host,
        props.sourceServer.port,
        props.sourceServer.user || '',
        props.sourceServer.password || '',
...
watch(() => props.sourceServer, (newServer) => {
  clonedServer.value = {
    ...
    database: newServer.database || 'postgres'
  };
}, { immediate: true });
</script>