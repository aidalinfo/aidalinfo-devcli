  ...
  <div class="space-y-4">
    ...
    <div v-if="filteredServers.length === 0" class="text-center py-8 bg-muted/50 rounded-lg">
      ...
      <p class="text-sm text-muted-foreground mb-3">
        {{ props.excludeServerId ? 'Aucun autre serveur MySQL disponible' : 'Aucun serveur MySQL configuré' }}
      </p>
    ...
    <div v-else class="space-y-4">
      <div class="flex justify-between items-center">
        <Label for="server-select" class="text-sm font-medium">Serveur MySQL :</Label>
...
const emit = defineEmits<{
  ...
  'server-selected': [server: MySQLServer];
}>();
...
const testConnection = async () => {
  ...
  const result = await MySQLServersManager.testConnection(selectedServer.value);
...
const loadServers = () => {
  ...
  if (props.autoSelectDefault && !props.modelValue && filteredServers.value.length > 0) {
    const defaultServer = MySQLServersManager.getDefaultServer();
